        ...
        <div class="bg-white rounded shadow">
          ...
          <form action="{{ route('categories.update',$category->id) }}" method="POST" id="form">
            ...
            <div class="px-6 pt-6 border-bottom border-secondary-light">
                ...
                <div class="mb-4">
                    <input class="w-full px-4 py-1 mb-2 text-sm placeholder-gray-500 bg-white border rounded" type="text" name="name" required value="{{ $category->name ?? '' }}">
                </div>
                <label class="text-sm font-medium mb-2" for="description">Description:</label>
                <div class="mb-4">
                    <textarea name="description" id="" cols="30" rows="10" class="w-full px-4 py-1 mb-2 text-sm placeholder-gray-500 bg-white border rounded">{{ $category->description ?? '' }}</textarea>
                </div>
